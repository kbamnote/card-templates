  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-violet-50 to-purple-50 text-gray-900">
      ...
      <section className="relative py-12 px-6">
        <div className="absolute inset-0 bg-gradient-to-r from-violet-500 to-purple-600 opacity-10 rounded-3xl transform rotate-3"></div>
        <div className="relative z-10 flex flex-col items-center text-center">
          <div className="w-36 h-36 rounded-full overflow-hidden border-4 border-white shadow-2xl -mt-8">
            <img
          ...
          <h1 className="text-3xl font-bold mt-6">{profileData?.name || "Alex Morgan"}</h1>
          <p className="text-violet-600 font-medium mt-2">{profileData?.profession || "Creative Director & Visual Artist"}</p>
          <p className="text-gray-600 mt-4 max-w-md">Creating meaningful visual experiences that blend artistry with strategy to help brands tell their story.</p>
      ...
      <section className="py-8 px-6 bg-white">
        <div className="grid grid-cols-2 gap-5">
          <div className="bg-violet-50 rounded-2xl p-5 text-center">
            <p className="text-3xl font-bold text-violet-600">250+</p>
            <p className="text-gray-600 mt-1">Projects</p>
          </div>
          <div className="bg-violet-50 rounded-2xl p-5 text-center">
            <p className="text-3xl font-bold text-violet-600">98%</p>
      ...
      <section className="py-8 px-6 bg-violet-50">
        <h2 className="text-2xl font-bold mb-4">Creative Services</h2>
        <div className="space-y-5">
          {services.map((service, index) => (
            <div key={index} className="bg-white rounded-3xl overflow-hidden shadow-md transform transition-transform hover:scale-[1.02]">
              <div className="p-6">
                <div className="flex">
                  <div className="w-16 h-16 rounded-2xl flex items-center justify-center mr-5" style={{ backgroundColor: `${accent}20` }}>
      ...
      <section className="py-8 px-6 bg-violet-50">
        <h2 className="text-2xl font-bold mb-4">Creative Packages</h2>
          ...
          {products.map((product, index) => (
            <div key={index} className="flex-shrink-0 w-72 bg-white rounded-2xl overflow-hidden shadow-sm">
              <div className="relative">
                ...
                />
                <div className="absolute top-4 right-4 bg-violet-600 text-white px-3 py-1 rounded-full text-sm font-bold">
                  Popular
                </div>
              </div>
              <div className="p-5">
                <h3 className="font-bold">{product.name}</h3>
                <p className="text-violet-600 font-bold mt-2" style={{ color: accent }}>{product.price}</p>
                <button 
      ...
      <section className="py-8 px-6 bg-white">
        <h2 className="text-2xl font-bold mb-4">Client Love</h2>
        <div className="space-y-5">
          {testimonials.map((testimonial, index) => (
            <div key={index} className="bg-violet-50 rounded-2xl p-6 relative">
              <div className="absolute top-0 right-0 w-16 h-16 rounded-bl-2xl" style={{ backgroundColor: accent }}>
                <Quote size={20} className="text-white m-3" />
              </div>
              <div className="flex items-center">
                <div className="w-12 h-12 rounded-full bg-violet-100 flex items-center justify-center">
                  <span className="font-bold text-violet-600">RC</span>
                </div>
                <div className="ml-3">
      ...
      <section className="py-8 px-6 bg-gradient-to-r from-violet-600 to-purple-700 text-white rounded-t-3xl">
        <h2 className="text-2xl font-bold mb-2">Let's Create Something Amazing</h2>
        <p className="text-violet-200 mb-5">Schedule a creative consultation to discuss your project</p>
        ...
        <div className="mt-6 grid grid-cols-1 gap-3">
          <div className="flex items-center bg-white/20 p-3 rounded-lg">
            ...
            <span>{profileData?.email || "hello@pixelperfectstudio.com"}</span>
          </div>
          <div className="flex items-center bg-white/20 p-3 rounded-lg">
            <Phone size={18} className="mr-3" />
            <span>{profileData?.phone1 || "+1 (555) 567-8901"}</span>
          </div>
          <div className="flex items-center bg-white/20 p-3 rounded-lg">
